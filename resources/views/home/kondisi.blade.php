...
<nav class="flex items-center justify-between px-6 md:px-12 py-4 bg-bg-soft/80 backdrop-blur-md sticky top-0 z-50 border-b border-mint-soft">
    ...
    <div class="hidden md:flex items-center gap-8 text-sm font-semibold text-ink-soft">
        ...
        <a href="{{ url('/laporan-kondisi') }}" class="text-primary">Laporan Kondisi</a>
...
<!-- ================= KONTEN LAPORAN KONDISI ================= -->
<section class="py-12 md:py-16 bg-bg-soft min-h-screen font-sans">
    <div class="container mx-auto px-6 md:px-12 max-w-7xl">

        <h1 class="text-3xl md:text-4xl font-extrabold text-ink mb-2">Laporan Kondisi Alat</h1>
        <p class="text-ink-soft mb-8">Rekapitulasi kondisi inventaris saat ini.</p>
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-bg-soft/70 border-b border-mint-soft">
                        ...
                        <th class="px-8 py-4 text-sm font-bold text-ink-soft uppercase tracking-wide">Kondisi Terakhir</th>
                        <th class="px-8 py-4 text-sm font-bold text-ink-soft uppercase tracking-wide">Tanggal Cek</th>
    ...
    @endforelse
        ...
        @if(isset($laporanKondisi) && method_exists($laporanKondisi, 'links'))
            <div class="mt-8">
                {{ $laporanKondisi->links() }}
            </div>
        @endif
