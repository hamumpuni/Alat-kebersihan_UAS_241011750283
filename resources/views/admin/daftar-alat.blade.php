            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-bg-soft/70 border-b border-mint-soft">
                        ...
                        <th class="px-6 py-4 text-sm font-bold text-ink-soft uppercase tracking-wide">Gambar</th>
                        <th class="px-8 py-4 text-sm font-bold text-ink-soft uppercase tracking-wide">Nama Alat</th>
                        <th class="px-8 py-4 text-sm font-bold text-ink-soft uppercase tracking-wide">Jenis</th>
                        <th class="px-8 py-4 text-sm font-bold text-ink-soft uppercase tracking-wide">Lokasi Penyimpanan</th>
                        <th class="px-8 py-4 text-sm font-bold text-ink-soft uppercase tracking-wide">Kondisi</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($daftarAlat as $index => $alat)
                        <tr class="border-b border-mint-soft last:border-0 hover:bg-bg-soft/40 transition-colors">
                            ...
                            <td class="px-6 py-5">
                                ...
                                @else
                                    <div class="w-11 h-11 rounded-lg bg-bg-soft flex items-center justify-center text-ink-soft">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14M14 8h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                    </div>
                                @endif
                            </td>
                            <td class="px-8 py-5 font-bold text-ink">{{ $alat->nama_alat }}</td>
                            <td class="px-8 py-5 text-ink-soft">{{ $alat->jenis }}</td>
                            <td class="px-8 py-5 text-ink-soft">{{ $alat->lokasi_penyimpanan }}</td>
                            <td class="px-8 py-5">
                                @php
                                    $warnaKondisi = match($alat->kondisi) {
                                        'Baik' => 'bg-green-100 text-green-700',
                                        'Rusak Ringan' => 'bg-orange-100 text-orange-700',
                                        'Perlu Perbaikan' => 'bg-yellow-100 text-yellow-700',
                                        'Rusak Berat' => 'bg-red-100 text-red-600',
                                        default => 'bg-gray-100 text-gray-600',
                                    };
                                @endphp
                                <span class="{{ $warnaKondisi }} text-xs font-bold px-3 py-1.5 rounded-full">{{ $alat->kondisi }}</span>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-8 py-10 text-center text-ink-soft">Belum ada data alat yang tercatat.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
